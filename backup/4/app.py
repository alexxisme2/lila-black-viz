...
with t_replay:
    ...
    with s_col:
        if not replay_mid:
            ...
            roster = '<div style="display:flex;flex-wrap:wrap;gap:6px 14px;padding:8px 2px;background:#07090d;border-top:1px solid #141c28">'
            roster += '<span style="font-family:IBM Plex Mono,monospace;font-size:0.58rem;color:#2e3d52;text-transform:uppercase;letter-spacing:0.1em;align-self:center">Players: </span>'
            for uid in human_ids_r:
                c = player_colors_r[uid]
                roster += f'<span style="display:flex;align-items:center;gap:5px;font-size:0.72rem;color:{c}"><span style="width:10px;height:10px;border-radius:50%;background:{c};display:inline-block"></span>{uid[:14]}…</span>'
            if nb_r > 0:
                roster += '<span style="display:flex;align-items:center;gap:5px;font-size:0.72rem;color:#445566"><span style="width:18px;height:2px;background:#445566;display:inline-block"></span>Bots</span>'
            roster += '</div>'
...
with t_heat:
    ...
    with hc1:
        ...
        if hdf.empty:
            ...
            # Plague Inc bubbles — top hotspots
            if show_bubbles:
                flat = g_norm.flatten()
                valid_idx = np.where(~np.isnan(flat))[0]
                if len(valid_idx) > 0:
                    top_n = min(8, len(valid_idx))
                    top_idx = valid_idx[np.argsort(flat[valid_idx])[::-1][:top_n]]
                    for idx in top_idx:
                        iy, ix = np.unravel_index(idx, g_norm.shape)
                        pct = flat[idx]
                        sz  = max(28, min(70, pct * 6))
                        fig_h.add_trace(go.Scatter(
                            x=[float(xc[ix])], y=[float(yc[iy])],
                            mode="markers+text",
                            marker=dict(size=sz, color="rgba(255,255,255,0.12)",
                                        line=dict(color="rgba(255,255,255,0.6)", width=1.5)),
                            text=[f"{pct:.1f}%"],
                            textfont=dict(color="white", size=9, family="IBM Plex Mono"),
                            textposition="middle center",
                            hovertemplate=f"{pct:.2f}% of all {htype} events here<extra></extra>",
                            showlegend=False,
                        ))
            ...
            <div style="display:flex;gap:12px;align-items:center;padding:8px 2px;font-size:0.71rem;color:#6878a0;border-top:1px solid #141c28;margin-top:6px">
              <span>Scale:</span>
              <span style="width:12px;height:12px;border-radius:2px;background:rgba(0,200,255,0.5);display:inline-block"></span><span>Low %</span>
              <span style="width:12px;height:12px;border-radius:2px;background:rgba(80,255,120,0.7);display:inline-block"></span><span>Medium</span>
              <span style="width:12px;height:12px;border-radius:2px;background:rgba(255,200,0,0.9);display:inline-block"></span><span>High</span>
              ...
              <span style="display:inline-block;width:14px;height:14px;border-radius:50%;border:1px solid rgba(255,255,255,0.5);background:rgba(255,255,255,0.1)"></span><span>Hotspot bubble (% label)</span>
            </div>""", unsafe_allow_html=True)
